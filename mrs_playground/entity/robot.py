...
class Robot(Entity):
    ...
    def update(self, *args, **kwargs):
        ...
        robot_in_range = self._sensing.sense(
            state=self.state, target="robot", add_noise=True)
        animal_in_range = self._sensing.sense(
            state=self.state, target="animal", add_noise=False)
        u_t = np.zeros(2)
    ...
    def display(self, screen: pygame.Surface, debug=False):
        ...
        #                    tuple(self._pose), 180, 1)
        # pygame.draw.circle(screen, pygame.Color("red"),
        #                    tuple(self._pose), 170, 1)
        # pygame.draw.circle(screen, pygame.Color("green3"),
        #                    tuple(self._pose), 155, 1)
